...
<head>
    ...
    <title>@yield('title', 'RepoBurla')</title>
    <script>
        (() => {
...
<body>
    ...
    <script>
        (() => {
            ...
            const reverbKey = @json(env('REVERB_APP_KEY'));
            const reverbHost = @json(env('REVERB_HOST', '127.0.0.1'));
            const reverbPort = Number(@json((int) env('REVERB_PORT', 8080)));
            const reverbScheme = @json(env('REVERB_SCHEME', 'http'));
            const notifSummaryUrl = @json(auth()->check() ? route('notificacoes.summary') : null);
